<x-app-layout>
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('hospital.update-requests') }}" class="px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50">
                ← Back to Requests
            </a>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
        ...
        <form method="POST" action="{{ route('hospital.update-request.approve-selected', $request->id) }}" x-data="{ selectAll: false }">
            @csrf
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-200">
                            <th class="text-left py-3 px-4 text-sm font-medium text-gray-700 w-16">
                                <input type="checkbox" x-model="selectAll" @change="document.querySelectorAll('input[name=\"fields[]\"]').forEach(cb => cb.checked = selectAll)" 
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-100 {{ $request->requested_full_name !== ($currentValues['full_name'] ?? '') ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="full_name" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        ...
                        <tr class="border-b border-gray-100 {{ $request->requested_dob?->format('Y-m-d') !== ($currentValues['date_of_birth'] ?? '') ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="date_of_birth" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        ...
                        <tr class="border-b border-gray-100 {{ $request->requested_blood_group !== ($currentValues['blood_group'] ?? '') ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="blood_group" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        ...
                        <tr class="border-b border-gray-100 {{ $request->requested_organ_needed !== ($currentValues['organ_needed'] ?? '') ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="organ_needed" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        ...
                        <tr class="border-b border-gray-100 {{ $request->requested_urgency_level !== ($currentValues['urgency_level'] ?? '') ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="urgency_level" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        ...
                        <tr class="border-b border-gray-100 {{ $request->requested_waiting_time !== ($currentValues['waiting_time'] ?? 0) ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="waiting_time" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        ...
                        <tr class="{{ $request->requested_other_organs != ($currentValues['organs_needed'] ?? []) ? 'bg-blue-50' : '' }}">
                            <td class="py-3 px-4">
                                <input type="checkbox" name="fields[]" value="other_organs" 
                                       class="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            ...
            <div class="mt-6 pt-6 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row gap-4">
                    ...
                    <!-- Approve All -->
                    <div class="flex-1">
                        <form method="POST" action="{{ route('hospital.update-request.approve', $request->id) }}" class="inline">
                            @csrf
    ...
    <form id="rejectForm" method="POST" action="{{ route('hospital.update-request.reject', $request->id) }}" class="hidden bg-white border border-gray-200 rounded-lg shadow-sm p-6 mb-6">
        @csrf
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
        <h3 class="text-base font-medium text-gray-900 mb-4">Request Information</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
                <p class="text-xs text-gray-500">Recipient</p>
                <p class="font-medium text-gray-900">{{ $request->recipient->user->name }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Submitted By</p>
                <p class="font-medium text-gray-900">{{ $request->requestedByUser->name }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500">Submitted At</p>
                <p class="font-medium text-gray-900">{{ $request->created_at->format('Y-m-d H:i') }}</p>
            </div>
            <div>
